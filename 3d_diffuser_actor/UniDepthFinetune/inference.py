...
def main():
    ...
    model = UniDepthV2Finetune(device, correction_head_weights='/home/yehhh/RL_2025_Spring_Final/3d_diffuser_actor/UniDepthFinetune/runs/correction_model.pth')

    model.pretrained.eval()
    model.correction_head.eval()

    files = sorted(glob.glob(os.path.join(npz_dir, '*.npz')))
    data = np.load(files[1200])
    cams = [
        'left_shoulder',
        'right_shoulder',
        'wrist',
