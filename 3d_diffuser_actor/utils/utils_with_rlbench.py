...
class RLBenchEnv:
    ...
    def __init__(
        ...
        self.obs_config = self.create_obs_config(
            image_size, apply_rgb, apply_depth, apply_pc, apply_cameras, depth_in_meters
        )
    ...
    def evaluate_task_on_multiple_variations(
        ...
        sum_var_success_rates = sum(var_success_rates.values())
        sum_var_num_valid_demos = sum(var_num_valid_demos.values())
        
        var_success_rates["mean"] = (
            sum(var_success_rates.values()) /
            sum(var_num_valid_demos.values())
        ) if sum_var_num_valid_demos > 0 else 0
            ...
            for step_id in range(max_steps):
                ...
                try:
                    ...
                    if output.get("trajectory", None) is not None:
                        ...
                        # execute
                        for id, action in enumerate(tqdm(trajectory)):
                            #try:
    ...
    def create_obs_config(
        self, image_size, apply_rgb, apply_depth, apply_pc, apply_cameras, depth_in_meters, **kwargs
    ):
        """
        Set up observation config for RLBench environment.
            ...
            :param depth_in_meters: Depth in meters.
