...
class VisionSensor(Object):
    ...
    # Note: the coordinate is OpenGL-style, so the focal length is negative.
    def get_focal_length(self):
        """
        Compute the focal lengths (fx, fy) in pixels based on resolution and perspective angle.
        
        Returns:
            (fx, fy): Tuple of focal lengths in pixels for x and y axes
        """
        res = np.array(self.get_resolution())  # [width, height]
        ratio = res[0] / res[1]  # aspect ratio: width / height
        pa_x = pa_y = math.radians(self.get_perspective_angle())  # convert to radians

        # Adjust perspective angles if the aspect ratio is not 1:1
        if ratio > 1:
        ...
        focal_lengths = -res / (2 * np.tan(persp_angles / 2))  # fx, fy in pixels
